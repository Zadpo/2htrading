<?php
    ...
    $page_no = issetsession('admin_cancel_orders_no') ? session('admin_cancel_orders_no') : 0;
    ...
    $total_records = ceil(sql_count_rows("SELECT * FROM orders WHERE status = 5 AND is_archived = 0") / $user['view']);

    switch(getaction()) {
        case "previous":
            if($page_no > 0)
                $page_no--;
            unset($_SESSION['admin_cancel_orders_no']);
            $_SESSION['admin_cancel_orders_no'] = $page_no;
            break;
        case "next":
        ...
        case "reset":
            unset($_SESSION['admin_cancel_orders_no']);
            $_SESSION['admin_cancel_orders_no'] = 0;
            $page_no = 0;
            break;
        default:
            break;
    ...
    $orders = sql_get_results("SELECT * FROM orders WHERE status = 5 AND is_archived = 0 ORDER BY id DESC LIMIT $page_no, {$user['view']}");

    if(strlen($user['name']) > 0) {
        $temp = sql_get_results("SELECT * FROM orders WHERE status = 5 AND is_archived = 0 LIMIT 0, {$user['view']}");
        $orders = array();
